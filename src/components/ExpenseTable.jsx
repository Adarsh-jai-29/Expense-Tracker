...
export default function ExpenseTable({ expense ,setExpense}) {
  ...
  return (
  <>  
    <ContextMenu menuPosition={menuPosition} setExpense={setExpense} rowId={rowId} />
        ...
        {filteredData.map(({ id, title, category, amount }) => {
          return (
            <tr key={id} onContextMenu={(e)=>{
              e.preventDefault()
          setMenuPosition({left:e.clientX,top:e.clientY,display:'block'})
          setRowId(id)
            }}>
              <td>{title}</td>
              <td>{category}</td>
